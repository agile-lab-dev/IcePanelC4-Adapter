...
compute_snapshot_description () {
    # extract the last message log, removes non-ascii characters, replaces non-alphanumeric characters with dashes, and take teh first 20 characters lowercase
    # shellcheck disable=SC1001
    message=$(git log -1 --pretty=%B | head -n 1 | tr -c \11\12\15\40-\176 | sed -r s/[^a-zA-Z0-9]+/-/g | sed -r s/^-+\|-+$//g | cut -c1-20 | tr '[:upper:]' '[:lower:]')
...
# these regex's are used to determine if the current location is a release branch (e.g. 'release/2.1') or a tag (e.g. 'v1.2.1')
...
# else if the location matches the release branch regex
...
# otherwise we are not in a tag location and neither on a release branch, so we will assume this is the master branch.
